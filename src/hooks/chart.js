...
function getDataHistory() {
    if (typeOfChart.value == 'line') {
        ...
        if (newDataHistory) {
            ...
            if (newDataHistory.length == 0) {       // Новый месяц показать только сумму
                let i = 1
    ...
    chart.value = new Chart(
        document.querySelector('#acquisitions'),
        {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: typeOfChart.value == 'line' ? 'Балланс' : months.value[chooseMonth.value][0],

                        data: typeOfChart.value == 'line' ? money.value : incomeExpenses.value,
                        backgroundColor: typeOfChart.value == 'line' ? 'rgb(39, 255, 39)' : 'rgba(75, 202, 87, 0.4)',
                        borderColor: typeOfChart.value == 'line' ? 'rgb(19, 100, 19)' : 'rgb(88, 202, 75)',
                        borderWidth: 1,
                        pointStyle: 'circle',
                        pointRadius: 5,
                        pointHoverRadius: 10,
